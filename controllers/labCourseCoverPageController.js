...
const getLabCourseCoverPage = async (req, res) => {
  try {
    ...
    // 🔸 Fetch course, faculty & section details
    const [rows] = await dbPool.query(
      `
      SELECT 
        c.course_id,
    ...
    // 🔸 Fetch saved edits from lab_course_cover_page
    const [savedData] = await dbPool.query(
      `
      SELECT * FROM lab_course_cover_page 
      WHERE offering_id = ? 
      LIMIT 1
      `,
      [offering_id]
...
const saveLabCourseCoverPage = async (req, res) => {
  try {
    ...
    // 🔸 UPSERT into lab_course_cover_page
    await dbPool.query(
      `
      INSERT INTO lab_course_cover_page (
        offering_id, faculty_id, course_id, courseName, courseCode, nbaCode,
        semesterSection, academicYear, courseCoordinator, coordinatorBatches,
        courseInstructor, instructorBatches
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
